...
- [ ] 1.1 Add `CONF_ELECTRICITY_DSO = "electricity_dso"` constant
- [ ] 1.2 Add `DEFAULT_ELECTRICITY_DSO = "Fluvius Zenne-Dijle"` constant
- [ ] 1.3 Add `ELECTRICITY_DSO_OPTIONS: list[str]` with the 25 DSO names from the
...
- [ ] 2.1 Replace `_RLP_URL_PATTERN` with the all-DSOs URL:
      `"https://www.synergrid.be/images/downloads/SLP-RLP-SPP/{year}/RLP0N%20{year}%20Electricity%20all%20DSOs.xlsb"`
- [ ] 2.2 Add `dso_name: str` parameter to `async_start()` (after `hass`)
- [ ] 2.3 Update `_async_download_and_parse()` to pass `dso_name` through to `_parse_xlsb`
- [ ] 2.4 Rewrite `_parse_xlsb(data, year, dso_name)`:
...
- [ ] 2.5 Update the HA Storage format to wrap weights under a `"weights"` key and add a
      `"dso"` key: `{"dso": dso_name, "weights": {...}}`
- [ ] 2.6 Update `_async_load()` to read the new format; if `raw.get("dso") != dso_name` or
      the format is the old flat dict (no `"dso"` key), discard and return `False`
      so that `async_start()` proceeds to re-download
- [ ] 2.7 Update `_async_persist()` to save in the new format

## 3. `config_flow.py` — DSO selector in electricity options

- [ ] 3.1 Import `CONF_ELECTRICITY_DSO`, `DEFAULT_ELECTRICITY_DSO`,
      `ELECTRICITY_DSO_OPTIONS` in the imports block
- [ ] 3.2 Add `CONF_ELECTRICITY_DSO` field to `_electricity_options_schema()` as the first
...
- [ ] 4.1 Import `CONF_ELECTRICITY_DSO` and `DEFAULT_ELECTRICITY_DSO` in `__init__.py`
- [ ] 4.2 In the electricity `async_setup_entry` path, read
      `dso = effective.get(CONF_ELECTRICITY_DSO, DEFAULT_ELECTRICITY_DSO)`
- [ ] 4.3 Pass `dso_name=dso` to `rlp_store.async_start(hass, dso_name=dso)`
