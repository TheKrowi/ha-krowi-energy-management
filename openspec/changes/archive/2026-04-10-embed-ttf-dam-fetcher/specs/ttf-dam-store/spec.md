...
The component SHALL maintain a `TtfDamStore` instance in `hass.data[DOMAIN]["ttf_dam_store"]` for the lifetime of the gas config entry. The store SHALL fetch daily TTF DAM prices from `https://mijn.elindus.be/marketinfo/dayahead/prices` with `market=GAS&granularity=DAY` and cache the results in memory. Query parameters: `from = (today - 30 days).isoformat()`, `to = today.isoformat()`.

The store SHALL read `response["statistics"]["averagePrice"]` and divide by `10` to obtain the 30-day `average` in `c€/kWh`. It SHALL sort `response["dataSeries"]["data"]` by `x` (ascending) and take the last entry's `y` divided by `10` as `today_price` in `c€/kWh`.
...
- **THEN** `SIGNAL_TTF_DAM_UPDATE` SHALL be dispatched
- **THEN** an ERROR-level log message SHALL be emitted

#### Scenario: Response missing expected keys
- **WHEN** the API returns a response without `dataSeries` or `statistics`
- **THEN** the store SHALL catch the `KeyError`, log at ERROR, and dispatch the signal
