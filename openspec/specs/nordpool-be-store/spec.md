...
The store SHALL subscribe to `async_track_time_change(hour=0, minute=0, second=1)`. When this fires, the store SHALL call `async_fetch_today()` to refresh today's price cache and clear the tomorrow cache.
...
- **THEN** the store's `data_tomorrow` SHALL be reset to `None` and `tomorrow_valid` set to `False`
...
The store SHALL expose the following computed properties derived from `data_today`:

| Property | Type | Description |
|---|---|---|
| `current_price` | `float \| None` | Value of the currently active 15-min slot |
| `average` | `float \| None` | Mean of all `value` entries in `data_today` (using `statistics.mean`) |
...
| `tomorrow` | `list[float]` | All 96 `value` entries for tomorrow in chronological order, or `[]` if not yet available |
