# Spec: synergrid-spp-store

## Purpose

Defines the `SynergridSPPStore` — a store that downloads and parses the annual Synergrid SPP (Synthetisch Productieprofiel) ex-ante weighting profile. The SPP represents a synthetic solar PV production profile concentrated on midday hours, used to compute the correct SPP-weighted monthly average export price as specified by variable-rate electricity suppliers (e.g. Mega).
