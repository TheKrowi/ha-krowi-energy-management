...
Defines the `GcvStore` — the internal store that fetches, caches, and serves monthly Gross Calorific Values (GCV) from the Atrias API for the configured GOS zone. The store is owned by the gas config entry and lives for its lifetime.
